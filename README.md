...
手头没有 ROM 的话，可以参考 [mengfei523060/nesrom](https://github.com/mengfei523060/nesrom)
—— 网友收集的一个 ROM 仓库。本项目**不附带任何 ROM**，请自行确认有权使用你下载的 ROM。

### 键位

两个人玩的时候**各占键盘一半**，互不打架；插上手柄会按 device 0/1/2/3 自动分给 1-4 号位。

| | 方向 | A | B | Select | Start |
|:---|:---|:---|:---|:---|:---|
| **1P（左半）** | **W A S D** | **K** | **J** | **U** | **I** |
| **2P（右半）** | **方向键** | **.** | **,** | **/** | **;** |
| **3P** | `T F G H` | `V` | `R` | `C` | `B` |
| **4P** | 小键盘 `8 4 5 6` | `Kp 1` | `Kp 3` | `Kp 0` | `Kp +` |

> 键位在打包后可以用 exe 同目录的 `godot-nes-input.cfg` 随便改（见下节）。
> 3/4 号手柄要游戏支持四人分插器（Four Score）才有意义（同样见下节）。
...
NES 本体只支持两个手柄，四个人玩要靠 **Four Score**（四人分插器）。本项目实现了它的读协议：
插上之后 `$4016` 先输出 1P 再输出 3P、`$4017` 先输出 2P 再输出 4P，数据在 bit0；
bit1 在前 8 位是 0、第 8-23 位是 1 —— 游戏就是靠这一位判断"插了分插器"。

**怎么开**：打包后的 exe 旁边会有一份 `godot-nes-input.cfg`（第一次运行自动生成），把
...
| 什么时候 | 配置文件放在哪 |
|:---|:---|
| 编辑器里运行 | `user://godot-nes-input.cfg`（Godot 的用户数据目录，不污染工程目录） |
| 打包成 exe 之后 | **exe 同一个目录**下的 `godot-nes-input.cfg` —— 改完重启生效，不用重新打包 |

配置文件就是文本，格式如下（键名用 Godot 的写法，`OS.get_keycode_string()` 那一套）：
...
文件不存在时会自动写一份带默认值的出来；认不出的键名会跳过并保留原绑定，不会让程序起不来。
配置里记着一个 `layout_version`：**改了默认键位的版本会把它 +1**，启动时发现配置的版本落后
就用新默认值重写一份 —— 否则旧配置会一直盖住新键位（本项目踩过这个坑）。
代价是升级那一次手工改过的键位会被重置，之后照旧以你的配置为准。
只替换**键盘**绑定，手柄（joypad）绑定保持不动。
...
全部测试都是**无头**的，不需要人看屏幕。测试 ROM 由脚本生成（**不含任何商业内容**），测试用的真实 ROM 自己放进 `tests/roms/` 即可（不入库；没有的话见上文那个 [ROM 仓库](https://github.com/mengfei523060/nesrom)）。
...
- [ ] 开放总线、PPUMASK 色彩强调
- [ ] 电池备份存档落盘
- [ ] **Four Score 的自动化测试**：现在协议的读 8+ 位（3P/4P 段和识别位）只有实现、没有
      自动化验证 —— 打算加一块 `input4p.nes`（读 24 位存进 RAM，再和手算期望比），
      顺便把 `--press` 扩展到能按 3/4 号手柄
